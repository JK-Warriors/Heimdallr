...
<form name="form" class="form-inline" method="get" action="<?php echo site_url('wl_tool/lock') ?>" >
  ...
  <a href="<?php echo site_url('wl_tool/lock') ?>" class="btn btn-warning"><i class="icon-repeat"></i> <?php echo $this->lang->line('reset'); ?></a>
 ...
 <?php foreach ($datalist  as $item):?>
    <tr style="font-size: 12px;">
        ...
        <td><center><?php echo check_db_status_level($item['connect'],$item['connect_tips']) ?></td>
				<td><center><a href="<?php echo site_url('wl_tool/lock_view?server_id='.$item['server_id'].'&db_type='.$item['db_type']) ?>"><?php echo $this->lang->line('lock_view'); ?></a></td>
	</tr>
 <?php endforeach;?>
 <?php }else{  ?>
<tr>
