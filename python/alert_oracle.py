...
g_alert = str(func.get_option('alarm'))
                ...
                if database_role=="PHYSICAL STANDBY":
                    if int(dg_delay) >= 3600*3 or int(mrp_status) < 1:
                        if int(dg_delay) >= 3600*3:
                            ...
                            func.add_alert(server_id,tags,host,port,create_time,db_type,'repl_delay',dg_delay,'warning','replication delay more than 3 hours',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
